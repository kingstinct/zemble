...
describe('advancedWithOr', () => {
  ...
  it('Should fail without array', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token' } })
  ...
  it('Should fail when roles array doesnt include the right role', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token', roles: [] } })
  ...
  it('Should succeed when the role includes admin role', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token', roles: [{ role: 'admin', organisationId: '123' }] } })
  ...
  it('Should succeed when the role includes superadmin and admin role', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token', roles: [{ role: 'admin', organisationId: '123' }, { role: 'superadmin', organisationId: '123' }] } })
  ...
  it('Should succeed when the role includes superadmin (and admin role for other org)', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token', roles: [{ role: 'admin', organisationId: '123' }, { role: 'superadmin', organisationId: '1234' }] } })
  ...
  it('Should fail when the role includes superadmin and admin role for the wrong organization', async () => {
    ...
    const token = signJwt({ data: { type: 'user-token', roles: [{ role: 'admin', organisationId: '1234' }, { role: 'superadmin', organisationId: '1234' }] } })
