...
export const addIngredient: NonNullable<MutationResolvers['addIngredient']> = async (_, { title, imageUrls, nutrientsPer100g, servingSizes, ingredientId }, { logger }) => {
  ...
  const ingredient: WithoutId<typeof Ingredient> = {
    ...
    nutrientsPer100g:
      nutrientsPer100g?.map(({ nutrient, amountInGrams }) => ({
        nutrient,
        amountInGrams,
      })) ?? [],
  }
  ...
  const eatable = await Eatables.findOneAndUpdate(
    { _id },
    {
      $set: ingredient,
      $setOnInsert: { _id },
