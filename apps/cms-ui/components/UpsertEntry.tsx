...
const UpsertEntry: React.FC<{
  ...
  const [, createEntry] = useMutation(
    useMemo(() => buildUpsertEntryMutation(entity), [entity]),
  )
  ...
  const [fieldForEntitySelection, setFieldForEntity] =
    useState<EntityRelationField | null>(null)
          ...
          if (field.__typename === 'EntityRelationField') {
            return (
              <Button
                ...
                icon='chevron-down'
              >
                {data[queryName][field.name]
                  ? data[queryName][field.name].displayName
                  : `Select ${field.name}`}
              </Button>
            )
          }
