...
export function TextControl<T extends FieldValues>({ control, name, rules, ...textInputProps }: Props<T>) {
  // eslint-disable-next-line @typescript-eslint/no-base-to-string
  const labelOrPlaceholder = (textInputProps.label ? textInputProps.label.toString() : textInputProps.placeholder) ?? ''
  const render = useCallback(
    ({ field: { onChange, onBlur, value } }: { readonly field: ControllerRenderProps<T, typeof name>; readonly fieldState: ControllerFieldState; readonly formState: UseFormStateReturn<T> }) => {
