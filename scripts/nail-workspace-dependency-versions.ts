...
const packageFiles = [...packageFilesUnfiltered].filter((filePath) => !filePath.includes('node_modules'))
  ...
  const processDeps = (deps: Record<string, string>) => {
    ...
    Object.entries(deps).forEach(([name, version]) => {
      if (version.startsWith('workspace:')) {
        const actualVersion = version === 'workspace:*' ? packageVersions[name] : version.replace('workspace:', '')
