...
test('should create a book', async () => {
  const createBookReq = await app.gqlRequestUntyped<
    ...
    unknown
  >('mutation { createBook(title: "Lord of the rings") { title } }', {}, opts)
...
test('should get books by id', async () => {
  const createBookReq = await app.gqlRequestUntyped<
    ...
    unknown
  >('mutation { createBook(title: "Lord of the rings") { id } }', {}, opts)
  ...
  const silmarillionCreateBookReq = await app.gqlRequestUntyped<
    ...
    unknown
  >('mutation { createBook(title: "Simarillion") { id } }', {}, opts)
  ...
  const { data } = await app.gqlRequestUntyped<
    ...
    unknown
  >(
    `query GetBooksById($ids: [ID!]!) { getBooksById(ids: $ids) { title } }`,
    {
      ids: [
...
test('should create a book with authors', async () => {
  ...
  const { data: jrr } = await app.gqlRequestUntyped<CreateAuthorMutationType>(
    `mutation CreateAuthor { createAuthor(firstName: "J.R.R.", lastName: "Tolkien") { id, firstName, lastName } }`,
    {},
    opts,
  )
  const { data: christopher } =
    await app.gqlRequestUntyped<CreateAuthorMutationType>(
      `mutation CreateAuthor { createAuthor(firstName: "Christopher", lastName: "Tolkien") { id, firstName, lastName } } `,
      {},
      opts,
    )
...
test('should search title field', async () => {
  ...
  const results = await app.gqlRequestUntyped<
    ...
    unknown
  >('query { searchBooks(query: "Lord of") { title } }', {}, opts)
...
test('should filter on title field', async () => {
  ...
  const results = await app.gqlRequestUntyped<
    ...
    unknown
  >(
    'query { filterBooks(title: { eq: "Lord of the rings" }) { title } }',
    {},
    opts,
  )
...
test('should get default hasKindleVersion', async () => {
  ...
  const results = await app.gqlRequestUntyped<
    ...
    unknown
  >(
    'query { searchBooks(query: "Lord of") { title hasKindleVersion } }',
    {},
    opts,
  )
