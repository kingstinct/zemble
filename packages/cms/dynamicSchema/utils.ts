...
function typeDeduper<T extends GraphQLUnionType | GraphQLObjectType>(
  type: T,
): T {
  if (types[type.name]) {
...
export const createTraverser = (entity: EntitySchemaType) => {
  ...
  const fieldValueMapper = (key: string, data: Record<string, unknown>) => {
    return Array.isArray(data[key])
      ...
      : entityRelationFieldNamesWithEntity[key]
        ? data[key]
        : data[key]
  }
  ...
  const mapArrayFields = (
    ...
    (Array.isArray(data) ? data : [data]).map(
      (el: Record<string, unknown>): Record<string, unknown> => {
        const ending = capitalize(Object.keys(el)[0]!)

        return {
          __typename:
            capitalize(entity.nameSingular) + capitalize(fieldName) + ending,
          ...traverseData(el),
        }
      },
    )
