...
import type { GraphQLInputObjectTypeConfig, GraphQLOutputType, GraphQLScalarType } from 'graphql'
import { GraphQLBoolean, GraphQLFloat, GraphQLID, GraphQLInputObjectType, GraphQLList, GraphQLObjectType, GraphQLString, GraphQLUnionType, Kind } from 'graphql'

import type { ArrayField, EntityRelationField } from '../graphql/schema.generated'
...
export const fieldToOutputType = (typePrefix: string, field: AnyField, relationTypes: Record<string, GraphQLObjectType>): GraphQLScalarType | GraphQLList<GraphQLOutputType> | GraphQLObjectType => {
...
export const fieldToInputType = (typePrefix: string, field: AnyField): GraphQLScalarType | GraphQLList<GraphQLInputObjectType> | GraphQLInputObjectType => {
  switch (field.__typename) {
    ...
    case 'ArrayField': {
      // eslint-disable-next-line no-case-declarations
      const availableFields = field.availableFields.reduce<GraphQLInputObjectTypeConfig>(
        (prev, f) => ({
          ...prev,
          fields: {
            ...prev.fields,
...
export const createTraverser = (entity: EntitySchemaType) => {
  ...
  const entityRelationFieldNamesWithEntity = {
    ...fields
      .filter((f) => f.__typename === 'EntityRelationField')
      .reduce(
        (prev, f) => ({
          ...prev,
    ...
    ...fields
      .filter((f) => f.__typename === 'ArrayField')
      .reduce(
        (prev, f) => ({
          ...(f as unknown as ArrayField).availableFields
            .filter((f) => (f as AnyField).__typename === 'EntityRelationField')
            .reduce(
              (prev, f) => ({
                ...prev,
                [f.name]: (f as EntityRelationField).entityNamePlural,
  ...
  const fieldValueMapper = (key: string, data: Record<string, unknown>) => {
    return Array.isArray(data[key]) ? mapArrayFields(key, data[key] as Record<string, unknown> | readonly Record<string, unknown>[]) : entityRelationFieldNamesWithEntity[key] ? data[key] : data[key]
  }

  const traverseData = (data: Record<string, unknown>) =>
  ...
  const mapArrayFields = (fieldName: string, data: Record<string, unknown> | readonly Record<string, unknown>[]) =>
