...
import Papr, { schema, types, VALIDATION_LEVEL } from 'papr'
...
class PaprWrapper {
  ...
  async contentCollection(name: string) {
    ...
    const model = this.papr?.models.get(name) as Model<
      EntityEntryType,
      (typeof EntityEntrySchema)[1]
    >

    if (!model)
      throw new Error(
        `Content collection "${name}" not found or not initialized`,
      )
    return model
  }
