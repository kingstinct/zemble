...
export const readEntities = async (): Promise<readonly EntitySchemaType[]> => {
  ...
  try {
    const schemaStr = await fsPromised.readFile(entityFilePath, {
      encoding: 'utf-8',
    })
    const contents = JSON.parse(schemaStr as string) as {
      readonly entities: readonly EntitySchemaType[]
    }
...
export const writeEntities = async (entities: readonly EntitySchemaType[]) => {
  ...
  await fsPromised.writeFile(
    entityFilePath,
    `${JSON.stringify({ entities, $schema: '../entities-json-schema.json' }, null, 2)}\n`,
