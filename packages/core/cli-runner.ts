...
if (
  ...
  zembleContext.logger.info(
    'Usage: zemble-dev <pathToPluginOrApp> [morePlugins..]',
  )
  process.exit(0)
}

if (pluginPaths.length === 0) {
  zembleContext.logger.warn(
    'No plugins specified so this is just an empty app, see `zemble-dev --help` for usage',
  )
}
...
export const cliRunner = async (runner: Runner) => {
  ...
  return runner({
    plugins: files.filter((f): f is Plugin => 'pluginName' in f),
  })
}
