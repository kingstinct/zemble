...
export const createApp = async ({ plugins: pluginsBeforeResolvingDeps, providerStrategies: providerStrategiesIn, appDir = process.cwd() }: Configure) => {
  ...
  const pluginsWithMiddleware = plugins.filter((plugin) => 'initializeMiddleware' in plugin)

  const logText = `Initializing ${packageJson.name} with ${plugins.length} plugins:\n${plugins
    .map((p) => `- ${p.pluginName}@${p.pluginVersion}${'initializeMiddleware' in p ? ' (middleware)' : ''}`)
    .join('\n')}`
