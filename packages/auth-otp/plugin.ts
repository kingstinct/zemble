...
interface OtpAuthConfig extends Zemble.GlobalConfig {
  ...
  readonly handleEmailAuthRequest?: (
    email: IEmail,
    twoFactorCode: string,
    context: Zemble.GlobalContext,
  ) => Promise<void> | void
  readonly handleSmsAuthRequest?: (
    phoneNum: E164PhoneNumber,
    twoFactorCode: string,
    context: Zemble.GlobalContext,
  ) => Promise<void> | void
  readonly generateTokenContents: ({
    ...
    phoneNumber,
    decodedToken,
  }: GenerateTokenContentArgs) =>
    | Promise<Omit<Zemble.OtpToken, 'iat'>>
    | Omit<Zemble.OtpToken, 'iat'>
}
...
type GenerateTokenContentArgs = {
  readonly email?: string
  readonly phoneNumber?: string
  readonly decodedToken?: TokenContents
}
...
const defaultConfig = {
  ...
  WHITELISTED_SIGNUP_EMAIL_DOMAINS: parseEnvJSON(
    'WHITELISTED_SIGNUP_EMAIL_DOMAINS',
    undefined,
  ),
  WHITELISTED_SIGNUP_EMAILS: parseEnvJSON(
    'WHITELISTED_SIGNUP_EMAILS',
    undefined,
  ),
  handleEmailAuthRequest: async (to, twoFactorCode) => {
    ...
    if (
      ...
      void sendEmail({
        ...
        subject: plugin.config.emailSubject
          ? simpleTemplating(plugin.config.emailSubject, {
              email: to.email,
              name: to.name ?? to.email,
              twoFactorCode,
            })
          : 'Login',
        text: plugin.config.emailText
          ? simpleTemplating(plugin.config.emailText, {
              email: to.email,
              name: to.name ?? to.email,
              twoFactorCode,
            })
          : `Your two factor code is ${twoFactorCode}`,
        html: plugin.config.emailHtml
          ? simpleTemplating(plugin.config.emailHtml, {
              email: to.email,
              name: to.name ?? to.email,
              twoFactorCode,
            })
          : `Your two factor code is <b>${twoFactorCode}</b>`,
        to,
      })
    }
    if (process.env.NODE_ENV === 'development') {
      plugin.providers.logger.info(
        `Generated code for ${to.email}: ${twoFactorCode}`,
      )
    }
  },
  ...
  handleSmsAuthRequest: async (to, twoFactorCode) => {
    ...
    const message = smsMessage
      ? simpleTemplating(smsMessage, { twoFactorCode })
      : `Your code is ${twoFactorCode}`
 ...
 */
const plugin = new Plugin<OtpAuthConfig, typeof defaultConfig>(
  import.meta.dir,
  {
    dependencies: [
      {
