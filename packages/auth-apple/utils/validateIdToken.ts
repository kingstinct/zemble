...
export const validateIdToken = async (idToken: string) => {
  try {
    const { payload } = await jose.jwtVerify<AppleJwtContents>(
      idToken,
      JWKS,
      {},
    )
    ...
    if (
      ...
      !plugin.config.skipEmailVerificationRequired
    ) {
      throw new GraphQLError(
        '[@zemble/auth-apple] User email not verified, either set skipEmailVerificationRequired to true or verify the email with Apple.',
      )
    }
