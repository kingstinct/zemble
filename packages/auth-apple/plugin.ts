...
import {
  type AppleUserSignupData,
  type AppleUserSignupDataOnWeb,
  generateBearerTokenFromAppleToken,
...
interface AppleAuthConfig extends Zemble.GlobalConfig {
  ...
  ) => Promise<Zemble.AppleToken> | Zemble.AppleToken
...
const defaultConfig = {
  ...
  UNAUTHENTICATED_REDIRECT_URL:
    process.env['AUTH_LOGIN_REDIRECT_URL'] ?? '/login',
...
const plugin = new Plugin<AppleAuthConfig, typeof defaultConfig>(
  import.meta.dir,
  {
    dependencies: [
      {
      ...
      app.hono.get(config.appleAuthInitializePath, async (ctx) => {
        ...
        if (!APPLE_CLIENT_ID) {
          return ctx.json(
            {
              error:
                'APPLE_CLIENT_ID needs to be set for Apple OAuth flow to work.',
            },
            500,
          )
        }
      ...
      app.hono.post(config.appleAuthCallbackPath, async (ctx) => {
        ...
        if (state) {
          ...
          self.providers.logger.error(
            'state is not present in formdata from Apple',
          )
          return ctx.redirect(UNAUTHENTICATED_REDIRECT_URL)
        }
        ...
        if (!idToken) {
          self.providers.logger.error(
            'No id_token found in formdata from Apple',
          )
          return ctx.redirect(UNAUTHENTICATED_REDIRECT_URL)
        }
        ...
        try {
          ...
          const userDataOnWeb = user
            ? (JSON.parse(user) as AppleUserSignupDataOnWeb)
            : undefined
          ...
          const bearerToken = await generateBearerTokenFromAppleToken(
            decoded,
            userData,
          )
          ...
          return ctx.redirect(
            `${AUTHENTICATED_REDIRECT_URL}?bearerToken=${bearerToken.bearerToken}`,
          )
        } catch (error) {
