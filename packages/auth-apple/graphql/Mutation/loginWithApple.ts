...
export const loginWithApple: NonNullable<
  MutationResolvers['loginWithApple']
> = async (
  _,
  {
    authorizationCode,
  ...
  if (state) {
    ...
    throw new GraphQLError(
      'state is required when enforceStateValidation is enabled',
    )
  }

  const idToken = await validateIdToken(identityToken)

  const { bearerToken, sub } = await generateBearerTokenFromAppleToken(
    idToken,
    {
