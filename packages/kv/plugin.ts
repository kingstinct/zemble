  ...
  middleware: async ({ app, config, logger }) => {
    await setupProvider({
      ...
      initializeProvider: async (pluginConfig) => {
        ...
        const redisClient = createClient(initWithConfig.redisUrl!, { redis: initWithConfig.redisOptions, logger })

        return function PrefixWrapper<T extends Zemble.KVPrefixes[K], K extends keyof Zemble.KVPrefixes = keyof Zemble.KVPrefixes>(prefix: K) {
