...
type Factory<
  TProps extends FactoryCustomProps,
  T extends StyleSheet.NamedStyles<T>,
> = (props: FactoryProps<TProps>) => StyleSheet.NamedStyles<T> | T

export function createThemedStylesHook<
  TProps extends FactoryCustomProps,
  TStyle extends StyleSheet.NamedStyles<TStyle>,
>(factory: Factory<TProps, TStyle>) {
  return (props?: TProps) => {
...
> = (props: FactoryProps<TProps>) => T
export function createThemedView<
  TProps extends FactoryCustomProps,
  TStyle extends StyleProp<ViewStyle>,
>(factory: ViewFactory<TProps, TStyle>) {
  const ThemedComponent: React.FC<PropsWithChildren<TProps>> = ({
    children,
    ...props
  }) => {
    const theme = useTheme(),
...
type TextFactory<
  TProps extends FactoryCustomProps,
  TStyle extends StyleProp<TextStyle>,
> = (props: FactoryProps<TProps>) => TStyle
export function createThemedText<
  TProps extends FactoryCustomProps,
  TStyle extends StyleProp<TextStyle>,
>(factory: TextFactory<TProps, TStyle>) {
  const ThemedComponent: React.FC<PropsWithChildren<TProps>> = ({
    children,
    ...props
  }) => {
