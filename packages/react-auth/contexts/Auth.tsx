...
const AuthProvider: React.FC<
  PropsWithChildren<{ readonly authTokenKey?: string }>
> = ({ children, authTokenKey = AUTH_TOKEN_KEY_DEFAULT }) => {
  const [{ token, isReady }, setToken] = useState<AuthStateInternal>({
    token: null,
    isReady: false,
  })
  ...
  const value = useMemo<AuthContextData>(
    () => ({
      ...
      status: isReady
        ? token
          ? Status.READY_WITH_TOKEN
          : Status.READY_WITHOUT_TOKEN
        : Status.INITIALIZING,
    }),
