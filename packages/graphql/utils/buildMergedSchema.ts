...
export const buildMergedSchema = async ({ plugins, appPlugin }: Pick<Zemble.App, 'plugins' | 'appPlugin'>, config: GraphQLMiddlewareConfig) => {
  const selfSchemas: readonly GraphQLSchemaWithContext<Zemble.GraphQLContext>[] = [
    // don't load if we're already a plugin
    ...(appPlugin ? (await createPluginSchema(appPlugin)).map((p) => p.schema) : []),
    // eslint-disable-next-line no-nested-ternary
    ...(config.extendSchema ? (typeof config.extendSchema === 'function' ? await config.extendSchema() : config.extendSchema) : []),
  ]
  ...
  const graphQLSchemas = await pluginsToAdd.reduce(async (prev, plugin) => {
    const toReturn: readonly GraphQLSchemaWithContext<Zemble.GraphQLContext>[] = [...(await prev), ...(await createPluginSchema(plugin)).map((p) => p.schema)]
