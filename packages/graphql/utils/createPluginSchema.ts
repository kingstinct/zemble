...
export const createPluginSchema = async (
  ...
  const [Query, Mutation, Subscription, types, scalars, typesOnRootLevel] =
    await Promise.all([
      readResolvers(
        join(graphqlDir, '/Query'),
        plugin.providers.logger,
        plugin.isPluginRunLocally,
      ),
      readResolvers(
        join(graphqlDir, '/Mutation'),
        plugin.providers.logger,
        plugin.isPluginRunLocally,
      ),
      readResolvers(
        join(graphqlDir, '/Subscription'),
        plugin.providers.logger,
        plugin.isPluginRunLocally,
      ),
      readResolvers(
        join(graphqlDir, '/Type'),
        plugin.providers.logger,
        plugin.isPluginRunLocally,
      ),
      readResolvers(
        join(graphqlDir, '/Scalar'),
        plugin.providers.logger,
        plugin.isPluginRunLocally,
      ),
      readResolvers(
  ...
  const schemaWithoutResolvers: GraphQLSchema | null = await loadSchema(
    graphqlGlob,
    {
    ...
    if (
      ...
      plugin.providers.logger.debug(
        `Error loading schema in ${graphqlGlob}:\n${e.message}`,
      )
      return null
    }
    throw e
