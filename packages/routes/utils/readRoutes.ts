...
export type PathsWithMetadata = Record<
  string,
...
export const readRoutes = async (
  {
    rootDir,
    logger,
  }: { readonly rootDir: string; readonly logger: IStandardLogger },
  prefix = '',
): Promise<PathsWithMetadata> =>
  fs.readdirSync(path.join(rootDir, prefix)).reduce(
    async (prev, filename) => {
      ...
      if (tat.isDirectory()) {
        const newRoutes = await readRoutes(
          { rootDir, logger },
      ...
      try {
        const newRoutes = {
          ...(await prev),
          [route]: { filename, relativePath: path.join(prefix, filename) },
        }
