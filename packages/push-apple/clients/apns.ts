...
export const makeRequest = async (body: ApnsBody, token: { readonly pushToken: string; readonly isSandbox?: boolean }, headerOptions?: HeaderOptions) => {
  const signedKey = await getBearerToken()

  const domain = token.isSandbox ? 'api.development.push.apple.com' : 'api.push.apple.com'
