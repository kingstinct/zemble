...
export const std = (arr: readonly number[], usePopulation = false) => {
  ...
  return Math.sqrt(arr.reduce((acc, val) => acc.concat((val - arrMean) ** 2), [] as readonly number[]).reduce((acc, val) => acc + val, 0) / (arr.length - (usePopulation ? 0 : 1)))
}
...
export const getStat = (arr: readonly number[], type: Statistic): number | null =>
  match(type)
    .with('AVG', () => {
...
type AnyPercentileOrTop = AnyPercentile | 'TOP_1' | 'TOP_2' | 'TOP_3'

export const isPercentile = (type: Statistic): type is AnyPercentile =>
  match(type)
    .with('AVG', () => false)
    .with('COUNT', () => false)
    .with('MAX', () => false)
...
export const isPercentileOrTop = (type: Statistic): type is AnyPercentileOrTop =>
  match(type)
    .with('AVG', () => false)
    .with('COUNT', () => false)
    .with('MAX', () => false)
...
export const getNumericPercentile = (type: AnyPercentile) =>
  match(type)
    .with('PERCENTILE_1', () => 1)
    .with('PERCENTILE_2', () => 2)
    .with('PERCENTILE_5', () => 5)
...
export const getNumericTop = (type: 'TOP_1' | 'TOP_2' | 'TOP_3') =>
  match(type)
    .with('TOP_1', () => 1)
    .with('TOP_2', () => 2)
    .with('TOP_3', () => 3)
...
export const isTop = (type: Statistic): type is 'TOP_1' | 'TOP_2' | 'TOP_3' =>
  match(type)
    .with('TOP_1', () => true)
    .with('TOP_2', () => true)
    .with('TOP_3', () => true)
