...
export type IdWithProjection<
  T extends Record<string, unknown> & { readonly _id: ObjectId },
> = {
...
export type Key<
  T extends Record<string, unknown> & { readonly _id: ObjectId },
> = IdWithoutProjection | IdWithProjection<T>

type OptimalProjection<
  T extends Record<string, unknown> & { readonly _id: ObjectId },
> = {
...
export function getOptimalProjection<
  T extends Record<string, unknown> & { readonly _id: ObjectId },
>(keys: readonly Key<T>[]): OptimalProjection<T> {
  const { projection, $in } = keys.reduce<OptimalProjection<T>>(
    (prev, cur) => ({
      ...prev,
      $in: [...prev.$in, getId(cur)],
      projection:
        prev.projection && cur.projection
          ? { ...prev.projection, ...cur.projection }
          : null,
    }),
    {
...
export type BatchLoadFnWithoutError<K, V> = (
  keys: ReadonlyArray<K>,
) => Promise<ArrayLike<V>>

export type MongoDataLoader<T extends PartialWithId<Record<string, unknown>>> =
  DataLoaderNullable<T, Key<T>, string>
...
export function createMongoDataloader<
  ...
  GQLPath extends Paths<GQLType> = Paths<GQLType>,
>(
  collection: Collection<EnhancedOmit<DBType, '_id'>>,
  dependencies?: Partial<Record<DBPath, readonly GQLPath[]>>,
): MongoDataLoader<DBType> {
    ...
    const finalProjection = projection
      ? {
          projection: dependencies
            ? removeDeepFields(handleExtraFields(projection, dependencies))
            : projection,
        }
      : {}

    const allDbUsers = await collection
      .find(
        {
          _id: { $in },
        },
    ...
    const orderedResult = keys.map<DBType | NotFoundError>((key) => {
      ...
      if (!dbUser) {
        return new NotFoundError(
          `${collection.collectionName} with _id: "${getId(key).toHexString()}" not found`,
        )
      }
