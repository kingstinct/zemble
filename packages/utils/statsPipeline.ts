import type { Collection, Join, NestedPaths, StrictFilter, WithId } from 'mongodb'
import { match } from 'ts-pattern'

import type { Timeoutify } from './node'
import type { Statistic } from './statHelpers'
import { getNumericPercentile, getNumericTop, isPercentile, isTop } from './statHelpers'
...
> {
  ...
  async execute(pipe: Record<string, unknown>[], timeoutify: Timeoutify) {
    // eslint-disable-next-line functional/prefer-readonly-type
    return timeoutify.runMongoOpWithTimeout(this.collection.aggregate(pipe)) as Promise<readonly TRes[]>
  }

  async executeWithMatch($match: StrictFilter<T>, timeoutify: Timeoutify) {
    // eslint-disable-next-line functional/prefer-readonly-type
    return timeoutify.runMongoOpWithTimeout(this.collection.aggregate(this.getPipeline($match), { allowDiskUse: true })) as Promise<readonly TRes[]>
  }

  getPipeline($match: StrictFilter<T>) {
    return [this.matchStage($match), ...this.preGroupStages, this.statisticsGroupStage, ...this.percentileSetStage, this.statisticsProjectStage]
  }
