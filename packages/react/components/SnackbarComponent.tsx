...
export const DefaultSnackbarComponent: React.FC<DefaultSnackbarComponentProps> = React.memo(({ snackbarConfig, textColor, textStyle, ...wrapperProps }) => (
...
export const DefaultSnackbarWrapper: React.FC<PropsWithChildren<DefaultSnackbarWrapperProps>> = React.memo(
  ({ snackbarConfig, doDismiss, backgroundColor, buttonColor, buttonTextStyle, id, style, entering, exiting, children, contentStyle }) => {
    const renderButton = useCallback(
      (a: Action, index: number) => (
        <TouchableOpacity
        ...
        >
          <Text style={[styles.buttonText, buttonTextStyle, buttonColor ? { color: buttonColor } : null, index === 0 ? null : styles.nonFirstButton]}>{a.label}</Text>
        </TouchableOpacity>
      ),
    ...
    return (
      ...
      >
        <View style={[styles.snackbar, style, backgroundColor ? { backgroundColor } : null]}>
          <View style={[styles.snackbarContent, contentStyle]}>{children}</View>
          <View style={styles.snackbarButtonWrapper}>{snackbarConfig.actions?.map(renderButton)}</View>
        </View>
      </Animated.View>
    )
  },
