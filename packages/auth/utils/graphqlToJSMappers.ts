...
export const getVariableReferenceSimple = (
  ...
  if (!argument) {
    throw new Error(
      `Could not find argument '${variableName}' in '${objectType.name}.${fieldNode?.name.value}'`,
    )
  }
  ...
  if (argument.value.kind === Kind.VARIABLE) {
    const valueFromVariable =
      executionArgs?.variableValues?.[argument.value.name.value]
...
export const handleValueNode = (
  ...
  if (value.kind === Kind.STRING) {
    if (value.value.startsWith('$')) {
      return getVariableReferenceSimple(value.value, {
        fieldNode,
        objectType,
        executionArgs,
      })
  ...
  if (value.kind === Kind.LIST) {
    return value.values.map((v) =>
      handleValueNode(v, { executionArgs, fieldNode, objectType }),
    )
  }
...
export const transformObjectNode = (
  ...
  return fields.reduce(
    (acc, field) => ({
      ...acc,
      [field.name.value]: handleValueNode(field.value, {
        executionArgs,
        fieldNode,
        objectType,
      }),
