...
export async function signJwt<T extends object>({
  ...
  if (!actualPrivateKey) {
    throw new Error(
      '[@zemble/auth] PRIVATE_KEY is not set, please set it as an environment variable or in the plugin config. You can run `bunx zemble-generate-keys` to add it to your .env',
    )
  }
