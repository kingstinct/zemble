...
import type { ExecutionArgs, FieldNode, GraphQLObjectType, ObjectValueNode } from 'graphql'
...
interface AuthConfig extends Zemble.GlobalConfig {
  ...
  readonly reissueBearerToken?: (bearerToken: TokenContents) => PromiseOrValue<TokenContents>
  readonly cookies?: {
      ...
      yoga: {
        plugins: [
          ...
          useGenericAuth<{ readonly decodedToken: Record<string, unknown> | null; readonly error?: GraphQLError }, Zemble.GraphQLContext>({
            ...
            validateUser: ({ fieldAuthDirectiveNode, user, fieldNode, objectType, executionArgs }) => {
              ...
              if (!decodedToken) {
                ...
                const skipArg = fieldAuthDirectiveNode?.arguments?.find((arg) => arg.name.value === 'skip')
                ...
                const matchArg = fieldAuthDirectiveNode?.arguments?.find((arg) => arg.name.value === 'match')
                ...
                const includesArg = fieldAuthDirectiveNode?.arguments?.find((arg) => arg.name.value === 'includes')
                ...
                const orArg = fieldAuthDirectiveNode?.arguments?.find((arg) => arg.name.value === 'or')

                if (orArg?.value.kind === Kind.LIST) {
                  const valid = orArg.value.values.some((value) => {
                    if (value.kind === Kind.OBJECT) {
                      const matchArg = value.fields.find((arg) => arg.name.value === 'match')
                      if (matchArg?.value.kind === Kind.OBJECT) {
                      ...
                      const includesArg = value.fields.find((arg) => arg.name.value === 'includes')
                      if (includesArg?.value.kind === Kind.OBJECT) {
