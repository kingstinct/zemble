...
describe('variableReference', () => {
  it('Should fail authentication', async () => {
    ...
    expect(response.errors?.[0]?.message).toEqual(
      `Accessing 'Query.variableReference' requires authentication.`,
    )
  })
  ...
  it('Should fail without right id in JWT', async () => {
    ...
    const token = await signJwt({
      data: { role: 'admin', organisationId: '2' },
      sub: '1',
    })
    ...
    expect(response.errors?.[0]?.message).toEqual(
      `Accessing 'Query.variableReference' requires token matching {"role":"admin","organisationId":"1"}.`,
    )
  })
  ...
  it('Should fail when trying to reference the exact variable literal', async () => {
    ...
    const token = await signJwt({
      data: { role: 'admin', organisationId: '$organisationId' },
      sub: '1',
    })
